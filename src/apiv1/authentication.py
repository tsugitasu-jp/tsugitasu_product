...
from drf_firebase_auth import __title__
...
class FirebaseAuthentication(authentication.TokenAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not getattr(api_settings, 'ALLOW_ANONYMOUS_REQUESTS') and not authorization_header:
            raise exceptions.MethodNotAllowed(None)
        try:
            token = authorization_header.split()[1].decode()
        except UnicodeError:
